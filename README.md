...
Major O&G operators run ICS/SCADA infrastructure at scale. A data engineer or analyst candidate who can demonstrate practical knowledge of **industrial protocols, time-series databases, and anomaly detection** stands out from candidates with generic ETL or business intelligence backgrounds.
...
```
Flow (m3/h):  raw_register / 10   => e.g., 1200 → 120.0 m3/h
...
| ------------------- | ---------- | --------- | ------- | --------- |
| Pump Flow           | 120.0 m3/h | +0.05     | 0.2     | 0–200     |
...
modbus_polling_loop() (The Async Driver)
Mechanism: Executed as a long-running, non-blocking asyncio.Task managed by the application’s core event loop.

Scheduling: Utilizes a periodic timer (typically implemented via await asyncio.sleep(1)) to execute deterministic 1 Hz (1-second interval) polling cycles.
...
Gas Compressor: Measures rotational speed and thermodynamic stress (bearing_temperature, rpm).
...
Network disruptions (network drops, DNS resolution failures, VPN lag) are an inevitable reality in industrial networks. To prevent data loss (0% data loss guarantee) when the TimescaleDB connection pool (asyncpg.pool) crashes during such disruptions, an SQLite-based Store-and-Forward architecture has been developed.

The engineering details of the mechanism are as follows:

Asynchronous Disk I/O (Non-Blocking): To avoid blocking the high-frequency MQTT data stream when writing to TimescaleDB fails, db_worker.py offloads the data to the synchronous sqlite3 library via a thread pool using loop.run_in_executor and writes it to the disk.

Dual-Channel Isolation: Sensor data (metrics_raw) and Anomaly engine outputs (system_alerts) have different schemas. While writing data to the SQLite local buffer (local_buffer.db), it is tagged with a target_table column.
